export default defineContentScript({
  ...
  main() {
    const notApplicableTags = [
      'HTML', 'HEAD', 'META', 'BASE', 'LINK', 'TITLE', 'SCRIPT', 'TEMPLATE',
      'CANVAS', 'STYLE', 'SVG', 'PATH', 'IMG', 'PICTURE', 'IFRAME', 'EMBED',
      'OBJECT', 'PARAM', 'VIDEO', 'AUDIO', 'SOURCE', 'TRACK', 'MAP', 'AREA',
      'MATH', 'BR', 'WBR'
    ]
    ...
    chrome.runtime.onMessage.addListener((_request, _sender, sendResponse) => {
      ...
      const htmlArea = htmlBg && !htmlBg.includes('rgba') ? htmlElement.clientWidth * htmlElement.clientHeight : 0
      const bodyArea = bodyBg && !bodyBg.includes('rgba') ? bodyElement.clientWidth * bodyElement.clientHeight : 0
      ...
      const elements = Array.from(document.getElementsByTagName('*')).filter(el =>
        !notApplicableTags.includes(el.tagName.toUpperCase())
      )
      ...
      const backgroundColors = countColors(allBackgroundColors.map(c => ({ color: c.color, value: c.area })))
      const textColors = countColors(allTextColors.map(c => ({ color: c.colorCode, value: c.area })))
    ...
    function getColorElement(values: Element[]): any[] {
      ...
      return elements.map(element => ({
        ...
        children: Array.from(element.children).filter(el => !notApplicableTags.includes(el.tagName.toUpperCase()))
      }))
    }
    ...
    function totalElementArea(elements: any[]): any[] {
      ...
      elements.forEach(el => {
        if (el.element.children.length === 0 || noChildrenTags.includes(el.element.tagName.toUpperCase())) return
    ...
    function countColors(data: { color: string | undefined; value: number }[]): { color: string; value: number }[] {
